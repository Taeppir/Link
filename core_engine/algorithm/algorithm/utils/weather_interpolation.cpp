...
// ===== 기존 parse_weather_data.cpp 코드 100% 보존 =====
    ...
    double lon)
{
    ...
    if (index != -1) {
        if (static_cast<size_t>(index) < it->second.data.size()) {
            ...
            // 특히 wave 관련 경우 -9999.0 아니면 음수(>0) 값들만 유효
            ...
            std::cerr << "  [Index Debug] FAILED calculated index " << index 
                      << " is out of data bounds." << std::endl;
            return 0.0;
        }
    }
    else {
